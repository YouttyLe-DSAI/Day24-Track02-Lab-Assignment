...
def validate_anonymized_data(filepath: str) -> dict:
    ...
    if len(set(df["cccd"]).intersection(set(raw_df["cccd"]))) > 0:
        results["success"] = False
        results["failed_checks"].append("Found raw CCCD in anonymized data")

    # Check 2: Không có null values trong các cột quan trọng
    # TODO: implement check
    if df[["patient_id", "cccd", "benh", "ket_qua_xet_nghiem"]].isnull().any().any():
        results["success"] = False
        results["failed_checks"].append("Null values found in critical columns")
